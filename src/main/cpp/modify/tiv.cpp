...
struct size {
  ...
  size fitted_within(size container) {
    double scale = std::min(container.width / (double) width, container.height / (double) height) * DISPLAY_FAC ;
    return scaled(scale);
  }
};
...
int main(int argc, char* argv[]) {
  ...
  cimg_library::CImg<unsigned char> image = load_rgb_CImg(argv[1]);

  printf("maxW*H: %d*%d\n", maxWidth, maxHeight);
  std::cout << "Src " << "W: " << image.width() << " " << "H: " << image.height() << std::endl;
  
  if (image.width() > maxWidth * DISPLAY_FAC || image.height() > maxHeight * DISPLAY_FAC) {
